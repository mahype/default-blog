	...
	foreach ( $post_types AS $post_type ):
		...
		if ( '' == $checked_posts ):
			if ( 'post' == $post_type->name ) {
				if ( is_array( $default_blog_template['posts'] ) ) {
					$checked_posts = $default_blog_template['posts'];
				}
			}
			if ( 'page' == $post_type->name ) {
				if ( is_array( $default_blog_template['pages'] ) ) {
					$checked_posts = $default_blog_template['pages'];
				}
			}
		endif;
...
function default_blog_posts_copy( $from_blog_id, $to_blog_id, $args = array() ) {
	...
	foreach ( $post_types AS $post_type ):
		...
		while ( $the_query->have_posts() ) : $the_query->the_post();
			...
			// Copy post
			default_blog_copy_post( $post->ID, $from_blog_id, $to_blog_id, array( 'copy_attachments'  => $copy_attachments,
			                                                                      'copy_comments'     => $copy_comments,
			                                                                      'copy_meta'         => $copy_meta,
			                                                                      'taxonomy_termlist' => $taxonomy_termlist
			) );
		endwhile;
