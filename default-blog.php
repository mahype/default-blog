...
class default_blog {
	...
	public function __construct() {
		...
		if ( is_admin() ):
			...
			add_action( 'after_setup_theme', array( $this, 'admin_load_framework' ), 1 );
			add_action( 'admin_init', array( $this, 'register_settings' ) );
			
			if ( isset( $_GET['page'] ) && ( 'defaultblog' == $_GET['page'] || 'options.php' == basename( $_SERVER['REQUEST_URI'] ) ) ):
	...
	function admin_menu() {
		add_submenu_page( 'sites.php', __( 'Default Blog', 'default-blog-options' ), __( 'Default Blog', 'default-blog-options' ), 'manage_options', 'defaultblog', array(
			$this,
			'admin_page'
		) );
	}
	...
	function get_options() {
		...
		if ( isset( $this->template_options[ DFB_TEMPLATE_ID ] ) && is_array( $this->template_options[ DFB_TEMPLATE_ID ] ) ) {
		...
		if ( isset( $this->template_options[ DFB_TEMPLATE_EDIT_ID ] ) && is_array( $this->template_options[ DFB_TEMPLATE_EDIT_ID ] ) ) {
